...
String kind="0";
totalRecord = rMgr.getReportCount_admin(id); 
...
<head>
<head>
		<title>받은 편지함 : Tomorrow is...</title>
		<meta charset="utf-8" />
...
<table>
  ...
  vlist = rMgr.getReportList(id,start, end); 
  ...
  %>
	  ...
	  for (int i = 0; i < numPerPage; i++) { 
		  ...
		  int idx = bean.getIdx(); 
		  String sender = rMgr.getSelectRepoter(idx); 
		  String reportMem = bean.getSender(); 
		  String title = bean.getTitle(); 
		  String contents = bean.getContents();
		  String date = bean.getDate(); 
		  String hits = bean.getCount();
		...
		<tr>
			...
			<td>
			  ...
			  for (int j = 0; j < pos; j++) { 
				  out.println("&nbsp;&nbsp;"); } } %> 
				  <a href="report_contents.jsp?idx=<%=idx%>"><%=title%></a>
				  <%if(hits.equals("0"))
				  	{
				  		%>
