<%@ page language="java" contentType="text/html; charset=EUC-KR"
	pageEncoding="EUC-KR"%>
<%@page import="sendBean"%>
<%@page import="java.util.Vector"%>
<jsp:useBean id="sMgr" class="sendMgr" />
...
<script type="text/javascript">
	function list() {
		document.listFrm.action = "main.jsp?pagefile=./sendList";
...
<body>
	...
	<table>
		...
		<% 
  vlist = rMgr.getBoardList(keyField, keyWord, start, end); 
  ...
  %>
		...
		<tr>
			...
			<td>
			  ...
			  for (int j = 0; j < depth; j++) { 
				  out.println("&nbsp;&nbsp;"); } } %> <a
				href="javascript:read('<%=idx%>')"><%=title%></a>
			</td>
			<td><%=mem_name%></td>
			<td><%=reg_date%></td>
		</tr>
	...
	<br />
	<input type="button"
		onClick="javascript:location.href='main.jsp?pagefile=./board/post'"
		value=" 글쓰기 ">
	<!-- 페이징 및 블럭 처리 Start-->
	<div class="paging">
		<ol>
			...
			if (totalPage != 0) { if (nowBlock > 1) { %>
			<li><a href="javascript:block('<%=nowBlock - 1%>')">[ 이전 ]</a> <%} %>&nbsp;</li>
			<% for (; pageStart < pageEnd; pageStart++) { %>
			<li><a href="javascript:pageing('<%=pageStart%>')"> <% if (pageStart == nowPage) { %>
					<font color="blue"><Strong> <% } %> [<%=pageStart%>]
							<% if (pageStart == nowPage) {%></Strong></font> <%} %></a></li>
			<% } //for %>&nbsp;
