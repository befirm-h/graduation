...
String kind="0";
totalRecord = sMgr.getTotalCount(id,kind); 
...
<head>
		...
		<title>보낸 고민함 : Tomorrow is...</title>
		<meta charset="utf-8" />
...
location.href="1.sendWorry.jsp?nowPage="+page;
}
function block(value) {
var pagePerBlock=<%=pagePerBlock%> * (value-1) + 1;
location.href="1.sendWorry.jsp?nowPage="+pagePerBlock;
		...
		<!-- Banner -->
			<section id="banner">
				<div class="inner">
					<div class="flex">					
						<div>
							<img src="images/icons/paper-plane.png" width="50px" /><br>
							<h3>보 낸 고 민 함</h3>
								<p><br>
							 </p>						
						</div>
...
<table>
		...
		<tbody>
		<% 
  vlist = sMgr.getSendList(id,kind, start, end); 
	  ...
	  for (int i = 0; i < numPerPage; i++) { 
		...
		<tr>
			...
			<td>
			  ...
			  for (int j = 0; j < pos; j++) { 
				  out.println("&nbsp;&nbsp;"); } } %> 
				  <a href="1.sendRead.jsp?idx=<%=idx%>"><%=title%></a>
			</td>
			<td><%=date%></td>
		</tr>
		<% } // for 
...
<input type="hidden" name="nowPage" id="nowPage" value="<%=nowPage%>">
	...
	<br><center>
						<input type="button" class="button" value="고민 비우기" onclick="location='1.send_contents.jsp'" >
